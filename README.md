...
Clients must authenticate with client credentials (client ID and secret) when issuing requests to `/oauth2/token` endpoint. Basic HTTP authentication should be used.
...
```sh
curl --compressed -v localhost:8000/oauth/v2/token \
...
```sh
curl --compressed -v localhost:8000/oauth/v2/introspect \
	-u test_client_1:test_secret \
